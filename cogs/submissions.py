...
class PeasantKindSelectView(discord.ui.View):
...
class PeasantKindSelect(discord.ui.Select):
    def __init__(self, original_message, prompt_msg, vision_data):
        ...
        opts = [
            ...
            discord.SelectOption(label="Peasant Performance", value="performance",
                                 description="You died — best single peasant life."),
        ]
        super().__init__(placeholder="Extraction or Performance?", options=opts)
    ...
    async def _bg_tasks():
        ...
        if selected_weapon and selected_class and not selected_class.startswith("Marksman"):
            try:
                ...
                # Auto-grant the Hundred-Handed role whenever the mark-sourced count
                # reaches 46. Re-checked on EVERY eligible submission (not only a new
                # combo) so already-complete legacy players get the role on their next run.
                try:
                    from cogs.leaderboards import HH_TOTAL as _HHT
                    _done = await _db.get_hh_done_combos(str(interaction.user.id), interaction.user.display_name)
                    if len(_done) >= _HHT:
        ...
        if any_updated:
            ...
            try:
                ...
                # Season window so Most Dominant / Warlord match the season standings card.
                _title_stats = await season_title_stats()
