...
class SubmissionsCog(commands.Cog):
    ...
    @app_commands.command(name="scan_missed", description="List a player's scorecard images the bot never tallied (reacted to) — for resubmitting (mod only).")
    @app_commands.describe(member="Whose posts to scan", limit="How many recent messages to scan (default 500, max 2000)")
    async def scan_missed(self, interaction: discord.Interaction, member: discord.Member, limit: int = 500):
        if not any(r.id == MOD_ROLE_ID for r in interaction.user.roles):
            await interaction.response.send_message("That's not for you.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
        limit = max(1, min(limit, 2000))
        channel = interaction.guild.get_channel(SUBMISSIONS_CHANNEL_ID) or await interaction.guild.fetch_channel(SUBMISSIONS_CHANNEL_ID)

        # Links already recorded as real submissions — belt-and-braces alongside
        # the reaction check (pre-bot posts have neither).
        try:
            logged = {(r[12] or "").strip() for r in await _db.get_all_submissions() if len(r) > 12 and r[12]}
        except Exception:
            logged = set()
        ...
        async for msg in channel.history(limit=limit):
            ...
            has_image = any(
                ...
                or (not a.content_type and a.filename.lower().endswith((".png", ".jpg", ".jpeg", ".webp", ".gif")))
                for a in msg.attachments
            )
            if not has_image:
                continue
            # "Tallied" = the cigar reaction the bot stamps on every processed
            # submission. Absence of the cigar (pre-bot posts, or ones that errored
            # out) means it was never counted. Match the cigar by emoji id.
            _CIGAR_ID = 1444893851427803298
            tallied = any(getattr(rx.emoji, "id", None) == _CIGAR_ID for rx in msg.reactions)
            if tallied or msg.jump_url in logged:
                continue
            missed.append(msg)

        if not missed:
            await interaction.edit_original_response(
                content=f"\u2705 Scanned {scanned} messages — no untallied scorecards found for **{member.display_name}**.")
            return

        missed.reverse()  # oldest first
        header = f"**{len(missed)} untallied image(s) from {member.display_name}** (scanned {scanned} msgs). Use `/reprompt <link>` on the real scorecards:\n"
        lines = [header]
        for m in missed:
            lines.append(f"\u2022 {m.created_at.strftime('%b %d %H:%M')} — {m.jump_url}")
        # Send in <2000-char chunks (jump links are long).
        chunk = ""
        first = True
        for ln in lines:
    ...
    @app_commands.command(name="reprompt", description="Re-post the submit prompt on an old scorecard image so it can be submitted (mod only).")
    @app_commands.describe(message_link="Jump link to the image message (from /scan_missed)")
    async def reprompt(self, interaction: discord.Interaction, message_link: str):
        if not any(r.id == MOD_ROLE_ID for r in interaction.user.roles):
            await interaction.response.send_message("That's not for you.", ephemeral=True)
            return
        await interaction.response.defer(ephemeral=True)
        try:
            ...
            await interaction.edit_original_response(content="\u274c Couldn't parse that message link.")
            return
        try:
            channel = interaction.guild.get_channel(ch_id) or await interaction.guild.fetch_channel(ch_id)
            msg = await channel.fetch_message(m_id)
        except Exception as e:
            await interaction.edit_original_response(content=f"\u274c Couldn't fetch that message: {e}")
            return
        view = SubmitView(msg)
        prompt = await msg.reply("\U0001f4cb Scorecard detected! Click below to submit your run.",
                                 mention_author=False, view=view)
        view.prompt_msg = prompt
        await interaction.edit_original_response(
            content=f"\u2705 Re-posted the submit prompt on {msg.jump_url} — **{msg.author.display_name}** can now click it to submit.")
