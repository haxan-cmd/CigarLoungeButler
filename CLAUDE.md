...
| `tests/` | Pure-logic tests (`pytest -q`, 101): parsing, ranks, tilt, feats, challenges, config integrity, boards, goals, rivalries, wrapped, validation, archetype (class + damage-style). Tests import only `utils/*` + `config` (never cogs — CI installs just pytest+dotenv). |
