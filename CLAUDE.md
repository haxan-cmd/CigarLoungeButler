...
| `tests/` | Pure-logic tests (`pytest -q`): parsing, ranks, tilt, feats, challenges, config integrity, boards, goals, rivalries, wrapped, validation, archetype, plus `test_regressions.py` (bugs that shipped — Score-as-TD, kills-companion, feat/archer classification), `test_grounding.py` (anti-fabrication) and `test_md_safe.py` (link-injection). Tests import only `utils/*` + `config` (never cogs — CI installs just pytest+dotenv). Eval methodology + the LLM-behavioural layer live in `docs/EVALS.md`. |
