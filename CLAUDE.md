...
| `utils/archetype.py` | Pure descriptive-playstyle label from a player's marks distribution (`derive_archetype` → "Knight Main", "Generalist", "Messer Specialist"). Neutral tone, single source. Shown on the registry card (`registry.archetype_label`) and injected into the Butler on data questions (`registry.get_player_archetype`). Unit-tested. |
